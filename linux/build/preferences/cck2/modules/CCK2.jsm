...
function alert(string) {
  Services.question.alert(Services.wm.getMostRecentWindow("navigator:browser"), "", string);
} 
...
//      Preferences.lock("distribution.about", String(config.id + " - " + config.version + " (CCK2)"));
      ...
      if (config.disableTelemetry) {
        ...
        Preferences.lock("toolkit.telemetry.asked", 999);
        ...
        try {
          ...
          UITour.onPageEvent = function(a, b) {
            ...
            if (aEvent.detail.action == "resetFirefox") {
              Services.question.alert(null, "CCK2", "This has been disabled by your administrator");
              return;
            }
            UITour.origOnPageEvent(a, b);
          }
          Preferences.lock("browser.disableResetquestion ", true);
        } catch (e) {}
      }
